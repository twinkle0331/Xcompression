...
path = "models/compressed_model" # change load_compressed_model as True please.
    ...
    for epoch in range(1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torchprof.Profile(model, use_cuda=False, profile_memory=True) as prof:
                loss = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next).item()
            print(prof.display(show_events=False))

            from pthflops import count_ops
            count_ops(model, (segment_ids, input_mask, lm_label_ids, is_next))
#             print(loss)
